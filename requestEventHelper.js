...
async function processClaimEventForWrapNonEVM(event, networkId) {
  ...
  try {
    ...
    logger.error("cannot decode token address", e);
    return;
  }
  ...
  let requestTx = db.Transaction.findOne({
    ...
    fromChainId: parseInt(event.returnValues._fromChainId),
    toChainId: parseInt(event.returnValues._toChainId),
    originChainId: parseInt(event.returnValues._originChainId),
...
async function getPastEventForBatchForWrapNonEVM(networkId, eventHookAddress, step, from, to) {
  let lastBlock = to
  let lastCrawl = from
  logger.info(`Wrap Network ${networkId} start batch from ${from} to ${to}`)
    ...
    try {
      ...
      for (let i = 0; i < allEvents.length; i++) {
        ...
        if (event.event === 'RequestBridge') {
          ...
          await processClaimEventForWrapNonEVM(event, networkId)
        }
      }
...
module.exports = {
  ...
  getPastEventForWrapNonEVM,
  getPastEventForBatchForWrapNonEVM,
  main
}
