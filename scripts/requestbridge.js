...
async function requestBridge() {
  try {
    ...
    if (erc20Address !== nativeAddress)
      await token.methods
        .approve(bridge.options.address, new BN("100000e18").toFixed(0))
        .send({
          chainId: web3.utils.toHex(fromChainId),
          from: mainAccount,
          gas: 1000000,
          gasPrice: gasPrice,
        });
    ...
    let encodedToAddress = web3.eth.abi.encodeParameters(["string"], [toAddr]);
    console.log("toAddr", toAddr);
    console.log(
      "encodedToAddress",
      Buffer.from(encodedToAddress).toString("hex")
    );

    await bridge.methods
      .requestBridge(erc20Address, encodedToAddress, amountInWei, toChainId)
      .send({
        chainId: web3.utils.toHex(fromChainId),
        from: mainAccount,
        gas: 1000000,
